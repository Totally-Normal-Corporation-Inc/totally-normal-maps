...
def main():
    ...
    for name, rows in payload['results'].items():
        ...
        for row in rows:
            ...
            if (name in {'totally_normal_maps/electoral-2026-09.json', 'totally_normal_maps/municipal-elections-2026-09.json'}
                    and row['type'] == 'Hex High Entropy String'
                    and LICENCE_CHECKSUM_LINE.fullmatch(lines[row['line_number'] - 1])):
                checksums += 1
                continue
